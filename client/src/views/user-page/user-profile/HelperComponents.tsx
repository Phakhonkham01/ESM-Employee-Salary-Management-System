...
export const InfoRow = ({
    ...
    <div className="flex justify-between items-center px-2 py-2 rounded hover:bg-sky-50 transition">
        <div className="flex items-center gap-2 text-slate-600">
            <span className="w-4 h-4 text-sky-400">{icon}</span>
            <span>{label}</span>
        </div>
        <span className={`font-semibold ${highlight ? 'text-emerald-500' : 'text-slate-800'}`}>
            {value}
        </span>
    </div>
...
export const ActionButton = ({
    ...
    return (
        <button
            onClick={onClick}
            className={`inline-flex items-center gap-2 px-4 py-2.5 rounded-lg font-medium transition-colors ${variant === 'primary'
